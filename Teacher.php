...
if(array_key_exists('action', $_GET) && $_GET['action'] == 'edit')
{
	...
	if(!empty($_POST))
	{
		...
		mysql_query('UPDATE `teacher` set `name` = "' . $name . '" WHERE `id`= ' . $id);

		header('Location: http://localhost/Attandence/Teacher.php');
	}

	$rsa = mysql_query('SELECT * FROM `teacher` WHERE `id`='  . $id);
...
if(array_key_exists('action', $_GET) && $_GET['action'] == 'delete')
{
	...
	mysql_query('DELETE FROM `teacher` WHERE `id` = ' . $id) or die (mysql_error());
...
if(!empty($_POST) && !array_key_exists('action', $_GET))
{
	...
	$sql = 'INSERT INTO `teacher` VALUES(' . $id .  ', "' . $name . '")';
