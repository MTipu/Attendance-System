...
if(array_key_exists('action', $_GET) && $_GET['action'] == 'edit')
{
	...
	if(!empty($_POST))
	{
		...
		header('Location: http://localhost/Attandence/Department.php');
	}

	$rsa = mysql_query('SELECT * FROM `department` WHERE `id`='  . $id);
...
if(array_key_exists('action', $_GET) && $_GET['action'] == 'delete')
{
	...
	mysql_query('DELETE FROM `department` WHERE `id` = ' . $id) or die (mysql_error());
